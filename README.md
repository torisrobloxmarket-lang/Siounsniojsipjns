...
-- Hinweis: Ersetze diesen Link mit dem Link zu deiner eigenen, monochromen Library-Datei (Datei 2)
...
UI.ESPBoxGrp:AddToggle("ESPCorner", { Text = "Corner ESP", Default = false })
    :AddColorPicker("ESPCornerColor", { Default = Color3.fromRGB(255, 255, 255), Title = "Corner Color" })
...
UI.ESPBoxGrp:AddToggle("ESPSkeleton", { Text = "Skeleton ESP", Default = false })
    :AddColorPicker("ESPSkeletonColor", { Default = Color3.fromRGB(200, 200, 200), Title = "Skeleton Color" })

UI.ESPBoxGrp:AddToggle("ESPHeadDot", { Text = "Head Dot", Default = false })
    :AddColorPicker("ESPHeadDotColor", { Default = Color3.fromRGB(255, 255, 255), Title = "Head Dot Color" })

UI.ESPBoxGrp:AddToggle("ESPTracer", { Text = "Tracer ESP", Default = false })
    :AddColorPicker("ESPTracerColor", { Default = Color3.fromRGB(200, 200, 200), Title = "Tracer Color" })
...
UI.ESPInfoGrp:AddToggle("ESPCharacter", { Text = "Character Moveset ESP", Default = false })
    :AddColorPicker("ESPCharacterColor", { Default = Color3.fromRGB(200, 200, 200), Title = "Character Color" })

UI.ESPInfoGrp:AddToggle("ESPDistance", { Text = "Distance ESP", Default = false })
    :AddColorPicker("ESPDistanceColor", { Default = Color3.fromRGB(200, 200, 200), Title = "Distance Color" })

UI.ESPInfoGrp:AddToggle("ESPKill", { Text = "Kill ESP", Default = false })
    :AddColorPicker("ESPKillColor", { Default = Color3.fromRGB(150, 150, 150), Title = "Kill Color" })
...
task.spawn(function()
    ...
    markerImg.ImageColor3 = Color3.fromRGB(255, 255, 255)
...
local function refreshPlayerDropdown()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Player then table.insert(vals, p.Name) end
    end
...
RunService:BindToRenderStep("RyuhubTargetLock", Enum.RenderPriority.Camera.Value + 5, function(dt)
    ...
    if Logic.LockState.FOVCircle then
        if Logic.LockSettings.ShowFOV and Logic.LockState.Enabled then
            ...
            Logic.LockState.FOVCircle.Color = Color3.fromRGB(255, 255, 255)
    ...
    if Logic.LockSettings.ESPEnabled and Logic.LockState.ESPGui then
        ...
        if espMain then espMain.Rotation = (tick() * 45) % 360 end
        local espMarker = Logic.LockState.ESPGui:FindFirstChild("Marker")
        if espMarker then espMarker.Position = UDim2.new(0.5, 0, -0.05, 0) end
    ...
    if Logic.LockSettings.ShowTargetInfo and Logic.LockState.TargetInfoName then
        ...
        if eHum then
            ...
            local hpCol = Color3.fromRGB(255, 255, 255)
        ...
        if model ~= char and model:IsA("Model") then
            ...
            if enemyRoot and enemyHum and enemyHum.Health > 0 then
                ...
                if dist < minDistance and dist <= maxDist then
                    minDistance = dist; closest = model
                end
            end
        end
    end
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if not checkcaller() and method == "FireServer" and self == remotes.Divergent and Toggles.BlackFlashEnabled and Toggles.BlackFlashEnabled.Value then
...
local function autoFireDivergentFist()
    ...
    if character and character:FindFirstChild("Moveset") then
        ...
        if divergentFistMove and remotes.Divergent then
            remotes.Divergent:FireServer(divergentFistMove, nil)
        end
    end
end
...
local function performDashLogic(target)
    ...
    dashConn = RunService.Heartbeat:Connect(function()
        ...
        if Toggles.CameraLockEnabled and Toggles.CameraLockEnabled.Value then
    ...
    if Toggles.BlackFlashEnabled and Toggles.BlackFlashEnabled.Value then
        ...
        if idMatch and Logic.TargetAnimations[idMatch] then
            if (tick() - Logic.LastFiredTick) <= Logic.TIME_WINDOW then
                ...
                local closestTarget = getClosestTarget(Options.DashDistance and Options.DashDistance.Value or 15)
                if closestTarget then
                    performDashLogic(closestTarget)
                else
                    task.delay(Options.FireDelay and Options.FireDelay.Value or 0.25, autoFireDivergentFist)
                end
            end
        end
    end
...
local function hpColor(f)
    f = math.clamp(f, 0, 1)
    return Color3.fromRGB(255 * (1 - f), 255 * f, 255 * f)
end
...
local function CreateCooldownBillboard(character, player, rootPart)
    ...
    EvasiveFill.BackgroundColor3 = Color3.fromRGB(200, 200, 200) 
...
Logic.Connections.ESP = RunService.Heartbeat:Connect(function()
    ...
    for _, data in ipairs(entities) do
        ...
        if Toggles.ESPBox and Toggles.ESPBox.Value and box then
            for i = 1, 4 do e.BoxL[i].Color = Options.ESPBoxColor and Options.ESPBoxColor.Value or Color3.fromRGB(255,255,255) end
        ...
        if Toggles.ESPCorner and Toggles.ESPCorner.Value and box then
            for i = 1, 8 do e.CorL[i].Color = Options.ESPCornerColor and Options.ESPCornerColor.Value or Color3.fromRGB(255,255,255) end
        ...
        if Toggles.ESPSkeleton and Toggles.ESPSkeleton.Value then
            ...
            for _, b in ipairs(bones) do
                ...
                if p1 and p2 then
                    ...
                    if o1 and o2 then
                        e.Skel[idx].Color   = Options.ESPSkeletonColor and Options.ESPSkeletonColor.Value or Color3.fromRGB(255,255,255)
                        e.Skel[idx].From    = s1
                        e.Skel[idx].To      = s2
        ...
        if Toggles.ESPHeadDot and Toggles.ESPHeadDot.Value and head then
            ...
            if ho then
                ...
                r = math.clamp(r, 3, 50)
                e.HDot.Color = Options.ESPHeadDotColor and Options.ESPHeadDotColor.Value or Color3.fromRGB(255,255,255)
        ...
        if Toggles.ESPTracer and Toggles.ESPTracer.Value then
            ...
            if fo then
                ...
                e.Trc.Color = Options.ESPTracerColor and Options.ESPTracerColor.Value or Color3.fromRGB(255,255,255)
        ...
        if Toggles.ESPHPText and Toggles.ESPHPText.Value and box then
        ...
        if Toggles.ESPHPBar and Toggles.ESPHPBar.Value and box then
            local f   = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
            local bx  = box.X - 6
        ...
        if Toggles.ESPOutline and Toggles.ESPOutline.Value then
            ...
            e.HL.OutlineColor = Options.ESPOutlineColor and Options.ESPOutlineColor.Value or Color3.fromRGB(255,255,255)
            e.HL.Enabled = true
        else
            if e.HL then e.HL.Enabled = false end
        end

        if Toggles.CooldownRevealer and Toggles.CooldownRevealer.Value then
        ...
        if box then
            local ay = box.Y - 2
            if Toggles.ESPCharacter and Toggles.ESPCharacter.Value then
                ay = ay - 14
                local ms = char:GetAttribute("Moveset") or (plr and plr:GetAttribute("Moveset")) or "Unknown"
                e.CharTxt.Color    = Options.ESPCharacterColor and Options.ESPCharacterColor.Value or Color3.fromRGB(255,255,255)
            ...
            if Toggles.ESPName and Toggles.ESPName.Value then
                ay = ay - 16
                e.NameTxt.Color    = Options.ESPNameColor and Options.ESPNameColor.Value or Color3.fromRGB(255,255,255)
            ...
            if Toggles.ESPDistance and Toggles.ESPDistance.Value then
                e.DistTxt.Color    = Options.ESPDistanceColor and Options.ESPDistanceColor.Value or Color3.fromRGB(255,255,255)
            ...
            if Toggles.ESPKill and Toggles.ESPKill.Value then
                local kills = 0
                if plr then
                ...
                e.KillTxt.Color    = Options.ESPKillColor and Options.ESPKillColor.Value or Color3.fromRGB(255,255,255)
...
local function refreshPlayerDropdown()
    ...
    pcall(function()
        if Options.LockSpecificPlayer then
            Options.LockSpecificPlayer:SetValues(vals)
            if not table.find(vals, Logic.LockSettings.SelectedPlayer) then
                Logic.LockSettings.SelectedPlayer = "Auto"
                Options.LockSpecificPlayer:SetValue("Auto")
...
Logic.Connections.DashInput = UserInputService.InputBegan:Connect(function(input, gp)
    ...
    local assignedKey = Options.DashAssistKeybind and Options.DashAssistKeybind.Value or Enum.KeyCode.J
    local targetKeyCode
...
local function performDashLogic(target)
    local MAX_DASH_DISTANCE = Options.DashDistance and Options.DashDistance.Value or 15
    local FIRE_DELAY = Options.FireDelay and Options.FireDelay.Value or 0.25
    local DASH_DURATION = Options.DashDuration and Options.DashDuration.Value or 0.35
    local POST_DASH_LOCK_TIME = Options.LockTime and Options.LockTime.Value or 0.1

    local easingStyleEnum = Enum.EasingStyle.Cubic
    local easingDirectionEnum = Enum.EasingDirection.Out
    ...
    if not root or not enemyRoot then
        task.delay(FIRE_DELAY, function() if remotes.Divergent then remotes.Divergent:FireServer(char.Moveset["Divergent Fist"], nil) end end)
        return
    end
    ...
    local function finalizeMovement(finalCFrame)
        ...
        if not hasFired then
            hasFired = true
            if remotes.Divergent and char:FindFirstChild("Moveset") and char.Moveset:FindFirstChild("Divergent Fist") then 
                remotes.Divergent:FireServer(char.Moveset["Divergent Fist"], nil) 
            end
        end
    end
...
ThemeManager:SetFolder("ryuhub")
SaveManager:SetFolder("ryuhub/tjs")
