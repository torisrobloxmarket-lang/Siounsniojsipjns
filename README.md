...
-- 1. TEST GUI
-- ============================================================================
local TestGui = Instance.new("ScreenGui", gethui and gethui() or game:GetService("CoreGui") or player.PlayerGui)
...
Title.Text = "Impel Down Tester (FULL)"
...
local getSpeed = function()
	return getMaxSpeed() > 0 and getMaxSpeed() - 5 or 110
end
...
local tweenTeleportPro = function(target)
	...
	if fixedY then
		...
		local raycastResult = Workspace:Raycast(Vector3.new(currentTarget.X, rayStartHeight, currentTarget.Z), Vector3.new(0, -20000, 0), raycastParams)
	...
	tween_connection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		if horizontalDistance < 8 then
			...
			if not fixedY then
				local raycastResult = Workspace:Raycast(Vector3.new(resolvedTarget.X, rayStartHeight, resolvedTarget.Z), Vector3.new(0, -20000, 0), raycastParams)
		...
		if fixedY then
			...
			local raycastResult = Workspace:Raycast(rayOrigin, Vector3.new(0, -20000, 0), raycastParams)
...
local tweenToPos2 = function(target, speed, flightHeight, maxHeight, usePathfinding, showWaypoints)
	...
	(function()
		...
		while true do
			while true do
				...
				if not getgenv().fish_kaitun and not getgenv().auto_santa and not getgenv().auto_impel then
					shouldStop = true
				end
				if shouldStop then
					break
...
local getAimCFrame = function(characterModel, targets)
	local primaryPart = characterModel and characterModel.PrimaryPart
	if not primaryPart then
		return CFrame.new()
	end
	local positionX = primaryPart.Position.X
	local positionY = primaryPart.Position.Y
	local positionZ = primaryPart.Position.Z
	local sumX = 0
	local sumZ = 0
	local targetCount = 0
	for _, target in ipairs(targets) do
		if typeof(target) == "Instance" and target:IsA("BasePart") and target.Parent then
			sumX = sumX + target.Position.X
			sumZ = sumZ + target.Position.Z
			targetCount = targetCount + 1
		end
	end
	local rootPosition = Vector3.new(positionX, positionY, positionZ)
	if targetCount == 0 then
		local currentCamera = Workspace.CurrentCamera
		return CFrame.new(rootPosition, (currentCamera.CFrame * CFrame.new(0, 0, -1000)).Position)
	end
	local centerX = sumX / targetCount
	local centerZ = sumZ / targetCount
	local direction = Vector3.new(centerX, positionY, centerZ) - rootPosition
	if direction.Magnitude < 0.001 then
		direction = Vector3.new(0, 0, -1)
	end
	local unitDirection = direction.Unit
	local lookOffset = unitDirection * 1.5
	local lookTarget = rootPosition + unitDirection
	return CFrame.new(rootPosition + lookOffset, lookTarget)
end
...
-- ============================================================================
-- 5. ORIGINAL IMPEL DOWN UTILITIES
-- ============================================================================
local getPrompt = function(position)
	for _, envChild in next, Workspace.Env:GetChildren() do
		if envChild.Name == "Part" and envChild:IsA("BasePart") and (envChild.Position - position).Magnitude < 10 then
			return envChild.ProximityPrompt
		end
	end
end
...
-- ============================================================================
-- 6. THE ORIGINAL IMPEL DOWN FARM LOOP
-- ============================================================================
		...
		elseif not player.PlayerGui:FindFirstChild("diffChooser") then
			if getCurrentFloor() == 1 then
				...
				elseif not player.Backpack:FindFirstChild("Bomb-Bomb") then
					...
					elseif not player.PlayerGui:FindFirstChild("ConfirmationPrompt") then
						...
						pcall(function() firesignal(game:GetService("Players").LocalPlayer.PlayerGui.ConfirmationPrompt.Main.OptionsFrame.Accept.MouseButton1Click) end)
					end
				elseif player.Backpack:FindFirstChild("Bomb-Bomb") then
					...
					if hasZone() then
						...
						if Workspace.Effects.Zones:FindFirstChild("End") then
							...
							tweenTeleportPro({
								Position = Vector3.new(2880, 2783, -14473) + zoneOffset,
								Speed = 50,
								Stop = function()
									return getgenv().auto_impel
								end
			...
			elseif getCurrentFloor() == 2 then
				if isWithinDistance2(Vector3.new(3199, 2405, -20056), 50, true) then
					tweenTeleportPro({
						Position = Vector3.new(3201, 2384, -20276),
						Speed = 50,
						Stop = function()
							return getgenv().auto_impel
						end
