...
task.spawn(function()
    local tweenInfo = TweenInfo.new(2.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true)
    pcall(function() TweenService:Create(DiscordGradient, tweenInfo, {Offset = Vector2.new(1, 0)}):Play() end)
end)
...
local function ToggleInvis(state)
    ...
    if state then
        if not char:FindFirstChild("FakeRoot") then
            ...
            -- RealRoot unter der Map festhalten, damit der Server dich dort "parkt"
