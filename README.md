--// ==========================================
--// RYU HUB - RAZORBILL EDITION (V2)
--// 100% MONOCHROME CLEAN TEMPLATE + RAZOR LOGIC
--// ==========================================
...
NotificationContainer.Name = "RyuNotifications"; NotificationContainer.Size = UDim2.new(0, 260, 1, -40); NotificationContainer.Position = UDim2.new(1, -280, 0, 20); NotificationContainer.BackgroundTransparency = 1
local NotifLayout = Instance.new("UIListLayout", NotificationContainer)
NotifLayout.SortOrder = Enum.SortOrder.LayoutOrder; NotifLayout.VerticalAlignment = Enum.VerticalAlignment.Bottom; NotifLayout.Padding = UDim.new(0, 8)
...
function RyuNotify:Send(title, text, duration)
    ...
    AccentLine.Size = UDim2.new(0, 3, 0.8, 0); AccentLine.Position = UDim2.new(0, 4, 0.1, 0); AccentLine.BackgroundColor3 = Color3.fromRGB(255, 255, 255); AccentLine.BackgroundTransparency = 1
    ...
    task.spawn(function()
        ...
        TweenService:Create(Stroke, TweenInfo.new(0.3), {Transparency = 1}):Play(); TweenService:Create(AccentLine, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
        TweenService:Create(TitleLabel, TweenInfo.new(0.3), {TextTransparency = 1}):Play(); TweenService:Create(DescLabel, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
...
ToggleBtn.Size = UDim2.new(0, 50, 0, 50); ToggleBtn.Position = UDim2.new(0, 15, 0, 60); ToggleBtn.BackgroundColor3 = Theme.Sidebar; ToggleBtn.Text = ""
...
MainFrame.Size = UDim2.new(0, 0, 0, 0); MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0); MainFrame.BackgroundColor3 = Theme.Background; MainFrame.BorderSizePixel = 0; MainFrame.Visible = false; MainFrame.ClipsDescendants = true; MainFrame.Active = true
...
SubTitle.Size = UDim2.new(0, 300, 0, 15); SubTitle.Position = UDim2.new(0, 20, 0, 38); SubTitle.BackgroundTransparency = 1; SubTitle.Text = "Razorbill Logic Integration"; SubTitle.TextColor3 = Theme.SubText; SubTitle.Font = Enum.Font.Gotham; SubTitle.TextSize = 11; SubTitle.TextXAlignment = Enum.TextXAlignment.Left
...
CloseBtn.MouseButton1Click:Connect(function()
    pcall(function() TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0.5, 0, 0.5, 0)}):Play() end); task.delay(0.3, function() MainFrame.Visible = false end)
end)
...
local function CreateSlider(section, text, min, max, default, callback)
    ...
    local sliderFill = Instance.new("Frame", sliderBg); local percentage = math.clamp((default - min) / (max - min), 0, 1); sliderFill.Size = UDim2.new(percentage, 0, 1, 0); sliderFill.BackgroundColor3 = Theme.Accent; Instance.new("UICorner", sliderFill).CornerRadius = UDim.new(1, 0)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            local value = (min + (max - min) * relative); local isDecimal = (math.floor(min) ~= min or math.floor(max) ~= max)
...
--// ==========================================
--// CONFIGURATION STATE (Replaces Old Options/Toggles)
--// ==========================================
local Config = {
    ...
    ESPHPText = false, ESPHPBar = false,
    CooldownRevealer = false,

    LockKeybind = Enum.KeyCode.C, EnableLock = false, LockESP = false, LockSpecificPlayer = "Auto",
    LockWallCheck = false, LockAutoRetarget = false, LockNotification = true,
    LockMethod = "Camera", LockTargetMode = "Closest", LockTargetPart = "HumanoidRootPart",
    LockSmoothness = 0, LockSideOffset = 1.75, LockMaxDistance = 500, LockPrediction = 0, TargetInfoColor = Color3.fromRGB(255, 255, 255),

    EnableDashAssist = false, DashCameraLock = false, DashOnlyIfFacing = false, DashAssistKeybind = Enum.KeyCode.J,
    DashDetectionRange = 15, DashBehindDistance = 5, DashFlightDuration = 0.42, DashCurveStrength = 10, DashArchHeight = 3, DashLockDuration = 0.35,

    TeleportDestination = "Town of Beginnings", TeleportMode = "Tween", TeleportSpeed = 350, TeleportVisualize = true,

    ServerDropdown = "Click 'Refresh' to load servers", ServerSortBy = "Default (Roblox)", MinPlayersFilter = 0, MaxPingFilter = 999
}

--// ==========================================
--// RAZORBILL LOGIC ENGINE (100% MIGRATED)
--// ==========================================
local Logic = {
...
local TeleportLocations = {
    ["Under the Map"]      = Vector3.new(-20.23, -61.53, -146.34), ["Unlicensed Studios"] = Vector3.new(196.86, 23.58, -37.27), ["Towers"]             = Vector3.new(25.35, 183.08, 110.77),
    ["Train Button"]       = Vector3.new(182.21, -9.33, 562.54), ["Bowling"]            = Vector3.new(267.60, -59.89, -255.06), ["Restaurant"]         = Vector3.new(-43.24, 23.63, -83.07),
    ["Storage House"]      = Vector3.new(195.69, 23.58, 151.44), ["Sewers 1"]           = Vector3.new(-148.14, -31.48, -127.22), ["Train Station"]      = Vector3.new(185.27, -9.69, -97.17),
    ["Sewers 2"]           = Vector3.new(60.84, -10.58, 167.47), ["Shenanigans Mall"]   = Vector3.new(155.66, -26.38, -254.85), ["Rhythm Game"]        = Vector3.new(12.23, -30.21, -315.03),
    ["Piano"]              = Vector3.new(-86.38, 26.65, -252.48), ["Convenience Store"]  = Vector3.new(-247.51, 26.96, -116.64), ["Court"]              = Vector3.new(124.48, 23.78, -247.06),
    ["Graveyard"]          = Vector3.new(228.55, 23.68, -130.48), ["Train Station Exit"] = Vector3.new(1.52, 24.72, 396.06), ["Tze's"]              = Vector3.new(-55.30, 23.62, 245.42),
    ["Jail"]               = Vector3.new(-243.84, 23.58, 126.97),
}
local LocationNames = {}; for k, _ in pairs(TeleportLocations) do table.insert(LocationNames, k) end; table.sort(LocationNames)
Config.TeleportDestination = LocationNames[1]

-- INVISIBILITY ENGINE (Original Razorbill Script)
_G.MiscState = _G.MiscState or {}
local Invisibility = { Connections = {} }

function Invisibility.toggle(state, silent)
    local char = Player.Character; if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); local hrp = char:FindFirstChild("HumanoidRootPart"); local animator = hum and hum:FindFirstChildOfClass("Animator")
    ...
    if state then
        ...
        _G.MiscState.InvisibleFakeTorso = Instance.new("Part"); _G.MiscState.InvisibleFakeTorso.Name = "FakeTorso"; _G.MiscState.InvisibleFakeTorso.Size = Vector3.new(2,2,1); _G.MiscState.InvisibleFakeTorso.Transparency = 1; _G.MiscState.InvisibleFakeTorso.CanCollide = false; _G.MiscState.InvisibleFakeTorso.Anchored = true; _G.MiscState.InvisibleFakeTorso.Parent = workspace
        Camera.CameraType = Enum.CameraType.Custom; Camera.CameraSubject = _G.MiscState.InvisibleFakeTorso
        ...
        if not silent then
            RunService:BindToRenderStep("MeditationFocus", Enum.RenderPriority.Camera.Value - 1, function()
                ...
                local camCF = Camera.CFrame; local lookDir = Vector3.new(camCF.LookVector.X, 0, camCF.LookVector.Z)
        ...
        pcall(function()
            ...
            _G.MiscState.InvisibleTrack = animator:LoadAnimation(animObj); _G.MiscState.InvisibleTrack.Priority = Enum.AnimationPriority.Action4; _G.MiscState.InvisibleTrack:Play()
        ...
        RyuNotify:Send("Invisibility", "Invisibility Enabled", 2)
    else
        ...
        RyuNotify:Send("Invisibility", "Invisibility Disabled", 2)
    end
end
...
local function visualizePath(waypoints)
    clearPathVisualization(); if not Config.TeleportVisualize then return end
...
local function tweenToPosition(hrp, targetPos, speed)
    local distance = (hrp.Position - targetPos).Magnitude; if distance < 0.5 then return true end
...
local function stepToPosition(hrp, targetPos, speed)
    local startPos = hrp.Position; local direction = (targetPos - startPos); local distance = direction.Magnitude; if distance < 0.5 then return true end
...
local function stopPathfinding()
    ...
    local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart"); if hrp then hrp.Anchored = false end
...
local function startPathfinding(targetPos)
    ...
    task.spawn(function()
        while Logic.Pathfinding.Active do
            local speed = Config.TeleportSpeed
            ...
            local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart"); local humanoid = Player.Character and Player.Character:FindFirstChildOfClass("Humanoid")
            if not hrp or not humanoid then task.wait(1) continue end
            if (hrp.Position - targetPos).Magnitude < 6 then RyuNotify:Send("Teleport", "Destination reached!", 3); stopPathfinding(); break end
            local ok, _ = pcall(function() path:ComputeAsync(hrp.Position, targetPos) end)
            if not ok or path.Status == Enum.PathStatus.NoPath then RyuNotify:Send("Teleport", "No path found! Retrying...", 2); task.wait(1.5); continue end
            ...
            for i = 2, #batchedWP do
                ...
                local success
                if Config.TeleportSpeed > 500 then success = stepToPosition(hrp, batchedWP[i].Position, Config.TeleportSpeed) else success = tweenToPosition(hrp, batchedWP[i].Position, Config.TeleportSpeed) end
                if not success then completed = false break end
            end
            if blockedConn then blockedConn:Disconnect() end
            if completed and Logic.Pathfinding.Active then RyuNotify:Send("Teleport", "Destination reached!", 3); stopPathfinding(); break end
            task.wait(0.3)
        end
    end)
end
...
local function w2v(pos)
    local cam = workspace.CurrentCamera; if not cam then return nil, false end
...
local function getCharBounds(char)
    ...
    local hrp = char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso"); if not hrp then return nil end
    local head = char:FindFirstChild("Head"); local pos = hrp.Position
    local topY = head and (head.Position.Y + 0.5) or (pos.Y + 2.8); local botY = pos.Y - 3.1
    local topWorld = Vector3.new(pos.X, topY, pos.Z); local botWorld = Vector3.new(pos.X, botY, pos.Z)
    local topSP, topOn = cam:WorldToViewportPoint(topWorld); local botSP, botOn = cam:WorldToViewportPoint(botWorld)
    if not topOn or not botOn then return nil end
    local h = math.abs(botSP.Y - topSP.Y); local w = h * 0.5; local cx = (topSP.X + botSP.X) / 2; local my = math.min(topSP.Y, botSP.Y)
    return { X = cx - w / 2, Y = my, W = w, H = h, CX = cx, CY = (topSP.Y + botSP.Y) / 2 }
end

local function getEntities()
    local out = {}; local folder = workspace:FindFirstChild("Characters"); local targetsToSearch = folder and folder:GetChildren() or workspace:GetChildren()
...
local function mkLine(thick) local l = Drawing.new("Line"); l.Visible = false; l.Thickness = thick or 1; return l end
local function mkCircle(thick, filled) local c = Drawing.new("Circle"); c.Visible = false; c.Thickness = thick or 2; c.Filled = filled or false; c.NumSides = 30; return c end
local function mkText(size) local t = Drawing.new("Text"); t.Visible = false; t.Size = size or 14; t.Center = true; t.Outline = true; t.OutlineColor = Color3.new(0, 0, 0); t.Font = 2; return t end
...
local function CreateCooldownBillboard(character, player, rootPart)
    ...
    table.insert(connections, character.ChildAdded:Connect(function(child) if child.Name == "Moveset" then trackMoveset() end end)); trackMoveset()

    local function updateUltimate() local ultValue = math.clamp((player:GetAttribute("Ultimate") or 0) / 100, 0, 1); if UltFill then UltFill.Size = UDim2.new(1, 0, ultValue, 0) end end
    local function updateEvade() local evadeValue = math.clamp((character:GetAttribute("Evade") or 0) / 50, 0, 1); if EvasiveFill then EvasiveFill.Size = UDim2.new(1, 0, evadeValue, 0) end end
    ...
    local function Cleanup() if Reveal then Reveal:Destroy() end; for _, conn in ipairs(connections) do conn:Disconnect() end; table.clear(connections) end
...
local function setLinePair(shadow, line, from, to, vis) shadow.From = from; shadow.To = to; shadow.Visible = vis; line.From = from; line.To = to; line.Visible = vis end
...
Logic.Connections.ESP = RunService.Heartbeat:Connect(function()
    ...
    for _, data in ipairs(entities) do
        ...
        if Config.ESPBox and box then
            for i = 1, 4 do e.BoxL[i].Color = Config.ESPBoxColor end
        ...
        if Config.ESPCorner and box then
            for i = 1, 8 do e.CorL[i].Color = Config.ESPCornerColor end
        ...
        if Config.ESPSkeleton then
            ...
            for _, b in ipairs(bones) do
                ...
                if p1 and p2 then
                    ...
                    if o1 and o2 then e.Skel[idx].Color = Config.ESPSkeletonColor; e.Skel[idx].From = s1; e.Skel[idx].To = s2; e.Skel[idx].Visible = true
        ...
        if Config.ESPHeadDot and head then
            ...
            if ho then
                ...
                e.HDot.Color = Config.ESPHeadDotColor; e.HDotS.Position = hs; e.HDotS.Radius = r + 1; e.HDotS.Visible = true; e.HDot.Position = hs; e.HDot.Radius = r; e.HDot.Visible = true
        ...
        if Config.ESPTracer then
            ...
            if fo then
                local bot = Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y); e.Trc.Color = Config.ESPTracerColor; e.TrcS.From = bot; e.TrcS.To = fp; e.TrcS.Visible = true; e.Trc.From = bot; e.Trc.To = fp; e.Trc.Visible = true
            else e.TrcS.Visible = false; e.Trc.Visible = false end
        else e.TrcS.Visible = false; e.Trc.Visible = false end

        if Config.ESPHPText and box then
        ...
        if Config.ESPHPBar and box then
        ...
        if Config.ESPOutline then
            if not e.HL or e.HL.Parent ~= char then
                safeRM(e.HL); local hl = Instance.new("Highlight"); hl.FillTransparency = 1; hl.OutlineColor = Config.ESPOutlineColor; hl.OutlineTransparency = 0; hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; hl.Parent = char; e.HL = hl
            end
            e.HL.OutlineColor = Config.ESPOutlineColor; e.HL.Enabled = true
        else if e.HL then e.HL.Enabled = false end end

        if Config.CooldownRevealer then
        ...
        if box then
            local ay = box.Y - 2
            if Config.ESPCharacter then ay = ay - 14; local ms = char:GetAttribute("Moveset") or (plr and plr:GetAttribute("Moveset")) or "Unknown"; e.CharTxt.Color = Config.ESPCharacterColor; e.CharTxt.Position = Vector2.new(box.CX, ay); e.CharTxt.Text = "[" .. tostring(ms) .. "]"; e.CharTxt.Visible = true else e.CharTxt.Visible = false end
            if Config.ESPName then ay = ay - 16; e.NameTxt.Color = Config.ESPNameColor; e.NameTxt.Position = Vector2.new(box.CX, ay); e.NameTxt.Text = dName; e.NameTxt.Visible = true else e.NameTxt.Visible = false end
            local by = box.Y + box.H + 2
            if Config.ESPDistance then e.DistTxt.Color = Config.ESPDistanceColor; e.DistTxt.Position = Vector2.new(box.CX, by); e.DistTxt.Text = math.floor(dist) .. " studs"; e.DistTxt.Visible = true; by = by + 14 else e.DistTxt.Visible = false end
            if Config.ESPKill then
                local kills = 0
                ...
                e.KillTxt.Color = Config.ESPKillColor; e.KillTxt.Position = Vector2.new(box.CX, by); e.KillTxt.Text = "Kills: " .. tostring(kills); e.KillTxt.Visible = true
...
local function refreshPlayerDropdown()
    ...
    if Config._LockSpecificPlayerRefresh then Config._LockSpecificPlayerRefresh(vals) end
end
...
Logic.Connections.PlayerRemoving = Players.PlayerRemoving:Connect(function(plr)
    ...
    if Logic.LockState.CurrentLockTarget then
        ...
        if targetPlayer == plr then
            ...
            if Config.LockNotification then RyuNotify:Send("Target Lock", plr.Name .. " left the game. Target lost.", 3) end
        end
    end
end)
...
local function getLockTarget()
    ...
    if Config.LockSpecificPlayer ~= "Auto" then
        local selectedName = Config.LockSpecificPlayer; local isNPC = string.sub(selectedName, 1, 6) == "[NPC] "
        if isNPC then
                ...
                for _, model in ipairs(charsFolder:GetChildren()) do
                    if model.Name == npcName and model:IsA("Model") then
                        ...
                        if tHum and tHum.Health > 0 and tRoot then
                            ...
                            if dist <= Config.LockMaxDistance then
                                if Config.LockWallCheck then
            ...
            if tHum and tHum.Health > 0 and tRoot then
                ...
                if Config.LockTargetMode == "Closest" then
                    ...
                    if onScreen then
                        ...
                        if Config.ShowFOV and Config.FOVRadius > 0 then
...
local function executeDashArc(direction)
    ...
    local targetChar = getClosestTarget(Config.DashDetectionRange); local target = targetChar and targetChar:FindFirstChild("HumanoidRootPart")
    if not target then isDashingArc = false return end

    if Config.DashOnlyIfFacing then
    ...
    if animator then
        ...
        dashTrack = animator:LoadAnimation(animToUse); dashTrack.Priority = Enum.AnimationPriority.Action4; dashTrack:Play(0.05, 1, 1 / Config.DashFlightDuration)
    end
    ...
    local tween = TweenService:Create(progress, TweenInfo.new(Config.DashFlightDuration, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), { Value = 1 })
    ...
    RunService:BindToRenderStep(dashName, 20000, function()
        if not target or not target.Parent or not root then
            RunService:UnbindFromRenderStep(dashName); if root then root.Anchored = false end; if humanoid then humanoid.AutoRotate = true end; if dashTrack then dashTrack:Stop() end; if Config.DashCameraLock then Camera.CameraType = prevCamType end; isDashingArc = false return
        end
        root.Anchored = true; humanoid.AutoRotate = false; root.AssemblyLinearVelocity = Vector3.zero
        ...
        local p2 = tPos + (flatLook * -Config.DashBehindDistance); local midPoint = (p0 + p2) / 2; local p1 = midPoint + (flatRight * (Config.DashCurveStrength * sideMult)) + Vector3.new(0, Config.DashArchHeight, 0)
    ...
    tween.Completed:Connect(function()
        ...
        if Config.DashCameraLock then Camera.CameraType = prevCamType end
        ...
        RunService:BindToRenderStep(lockName, 20000, function()
            if tick() - lockStart > Config.DashLockDuration or not target or not target.Parent or not root then RunService:UnbindFromRenderStep(lockName); if root then root.Anchored = false; root.AssemblyLinearVelocity = Vector3.zero end; if humanoid then humanoid.AutoRotate = true end; isDashingArc = false return end
...
RunService:BindToRenderStep("RazorbillTargetLock", Enum.RenderPriority.Camera.Value + 5, function(dt)
    ...
    if Logic.LockState.FOVCircle then
        if Config.EnableLock and Config.ShowFOV then
            Logic.LockState.FOVCircle.Position = Vector2.new(Mouse.X, Mouse.Y); Logic.LockState.FOVCircle.Radius = Config.FOVRadius; Logic.LockState.FOVCircle.Color = Config.TargetInfoColor; Logic.LockState.FOVCircle.Filled = Config.FOVFilled; Logic.LockState.FOVCircle.Transparency = 1 - Config.FOVTransparency; Logic.LockState.FOVCircle.NumSides = Config.FOVSides; Logic.LockState.FOVCircle.Visible = true
        else Logic.LockState.FOVCircle.Visible = false end
    end

    if not Config.EnableLock or not isAlive then
    ...
    if Logic.LockState.CurrentLockTarget then
        ...
        if not targetAlive then
            ...
            if Config.LockAutoRetarget then Logic.LockState.LastTargetSearch = 0; if Config.LockNotification and deadName then RyuNotify:Send("Target Lock", deadName .. " eliminated. Retargeting...", 2) end
            elseif Config.LockSettings.Sticky then if Config.LockNotification and deadName then RyuNotify:Send("Target Lock", deadName .. " eliminated. Target lost.", 2) end end
        else
            ...
            if tRoot then
                local dist = (root.Position - tRoot.Position).Magnitude
                if dist > Config.LockMaxDistance then
                    if not Config.LockSettings.Sticky then Logic.LockState.CurrentLockTarget = nil; if Config.LockNotification then RyuNotify:Send("Target Lock", "Target out of range. Searching...", 2) end end
                end
            end
        end
    end

    if not Config.LockSettings.Sticky and Logic.LockState.CurrentLockTarget == nil then Logic.LockState.CurrentLockTarget = getLockTarget()
    ...
    if Logic.LockState.CurrentLockTarget then
        ...
        if newName ~= Logic.LockState.LastTargetName then Logic.LockState.LastTargetName = newName; if Config.LockNotification then RyuNotify:Send("Target Lock", "Locked onto: " .. newName, 2) end end
    end
    ...
    local targetPart = Logic.LockState.CurrentLockTarget:FindFirstChild(Config.LockTargetPart) or Logic.LockState.CurrentLockTarget:FindFirstChild("HumanoidRootPart")
    ...
    if Config.LockPrediction > 0 then
        local tRoot = Logic.LockState.CurrentLockTarget:FindFirstChild("HumanoidRootPart")
        if tRoot and tRoot:IsA("BasePart") then targetPos = targetPos + (tRoot.AssemblyLinearVelocity * Config.LockPrediction) end
    end

    if Config.LockESP and Logic.LockState.ESPGui then
    ...
    if Config.ShowTargetInfo and Logic.LockState.TargetInfoName then
        local viewport = Camera.ViewportSize; local infoColor = Config.TargetInfoColor; local eHum = Logic.LockState.CurrentLockTarget:FindFirstChildOfClass("Humanoid"); local tDist = (root.Position - targetPart.Position).Magnitude
        local baseX, baseY
        if Config.TargetInfoPos == "Top Center" then baseX = viewport.X / 2; baseY = 35
        elseif Config.TargetInfoPos == "Bottom Center" then baseX = viewport.X / 2; baseY = viewport.Y - 90
        elseif Config.TargetInfoPos == "Top Right" then baseX = viewport.X - 150; baseY = 35
    ...
    if Config.LockMethod == "Camera" then
        ...
        local lookCF = CFrame.lookAt(camPos, targetPos); camPos = camPos + (lookCF.RightVector * Config.LockSideOffset)
        ...
        if Config.LockSmoothness <= 0 then Camera.CFrame = desiredCF else local rate  = 30 / Config.LockSmoothness; local alpha = math.clamp(1 - math.exp(-rate * dt), 0, 1); Camera.CFrame = Camera.CFrame:Lerp(desiredCF, alpha) end
    else
...
local function pingQualityIcon(ping) if ping <= 60 then return "[+++]" elseif ping <= 120 then return "[++ ]" elseif ping <= 200 then return "[+  ]" else return "[!  ]" end end
...
local function fetchServers()
    ...
    RyuNotify:Send("Server Browser", "Fetching servers... This may take a moment.", 4)
    ...
    RyuNotify:Send("Server Browser", "Found " .. tostring(#Logic.ServerList) .. " servers!", 4)
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if not checkcaller() and method == "FireServer" and self == Logic.TargetRemote and Config.BlackFlashEnabled then
...
local function performDashLogic(target)
    local MAX_DASH_DISTANCE = Config.DashDistance; local FIRE_DELAY = Config.FireDelay; local DASH_DURATION = Config.DashDuration; local POST_DASH_LOCK_TIME = Config.LockTime
    local easingStyleEnum = Enum.EasingStyle[Config.DashEasingStyle] or Enum.EasingStyle.Cubic; local easingDirectionEnum = Enum.EasingDirection[Config.DashEasingDirection] or Enum.EasingDirection.Out
    ...
    dashConn = RunService.Heartbeat:Connect(function()
        ...
        if Config.CameraLockEnabled then local cam = workspace.CurrentCamera; if cam then cam.CFrame = CFrame.lookAt(cam.CFrame.Position, initialEnemyPos) end end
...
local function onAnimationPlayed(animTrack)
    ...
    if Config.EnableDashAssist and not isDashingArc then
    ...
    if Config.BlackFlashEnabled then
        ...
        if idMatch and Logic.TargetAnimations[idMatch] then
            if (tick() - Logic.LastFiredTick) <= Logic.TIME_WINDOW then
                ...
                local closestTarget = getClosestTarget(Config.DashDistance)
                if closestTarget then performDashLogic(closestTarget) else task.delay(Config.FireDelay, autoFireDivergentFist) end
            end
        end
    end
end
...
Logic.Connections.DashInput = UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if not Config.EnableDashAssist then return end
    if input.KeyCode == Config.DashAssistKeybind then
...
UserInputService.InputBegan:Connect(function(input, gp)
    if gp then return end
    if input.KeyCode == Config.LockKeybind then
        Config.EnableLock = not Config.EnableLock
        if Config.LockNotification then
            if Config.EnableLock then RyuNotify:Send("Target Lock", "Lock ENABLED", 2)
            else RyuNotify:Send("Target Lock", "Lock DISABLED", 2) end
        end
    end
end)
...
CreateToggle(SecCombat, "Aktiviere Black Flash Chain", "Auto-Dash hinter Feinde bei Divergent Fist.", false, function(state) Config.BlackFlashEnabled = state end)
CreateToggle(SecCombat, "Kamera-Lock bei Dash", "Zwingt die Kamera auf den Rücken des Gegners.", true, function(state) Config.CameraLockEnabled = state end)
CreateSlider(SecCombat, "Max Dash Distanz", 5, 50, 15, function(val) Config.DashDistance = val end)
CreateSlider(SecCombat, "Combo Verzögerung (sek)", 0.1, 1.0, 0.25, function(val) Config.FireDelay = val end)
CreateSlider(SecCombat, "Dash Dauer (sek)", 0.1, 1.0, 0.35, function(val) Config.DashDuration = val end)
CreateSlider(SecCombat, "Post-Dash Lock Time", 0.0, 0.5, 0.1, function(val) Config.LockTime = val end)
CreateDropdown(SecCombat, "Dash Easing Style", {"Linear", "Sine", "Quad", "Cubic", "Quart", "Quint", "Expo", "Circ", "Elastic", "Back", "Bounce"}, "DashEasingStyle", function(val) Config.DashEasingStyle = val end)
CreateDropdown(SecCombat, "Dash Easing Richtung", {"In", "Out", "InOut"}, "DashEasingDirection", function(val) Config.DashEasingDirection = val end)
...
CreateToggle(SecESPBox, "Box ESP", "Zeigt eine Box um die Spieler an.", false, function(state) Config.ESPBox = state end)
CreateToggle(SecESPBox, "Corner ESP", "Zeigt Ecken um die Spieler an.", false, function(state) Config.ESPCorner = state end)
CreateToggle(SecESPBox, "Outline ESP", "Zeigt einen Rahmen um den Charakter.", false, function(state) Config.ESPOutline = state end)
CreateToggle(SecESPBox, "Skeleton ESP", "Zeigt das Skelett des Spielers an.", false, function(state) Config.ESPSkeleton = state end)
CreateToggle(SecESPBox, "Head Dot", "Macht einen Punkt auf den Kopf.", false, function(state) Config.ESPHeadDot = state end)
CreateToggle(SecESPBox, "Tracer ESP", "Zieht eine Linie vom Bildschirm zum Spieler.", false, function(state) Config.ESPTracer = state end)

local SubESPInfo = CreateSubTab(TabVisuals, "ESP Information")
local SecESPInfo = CreateSection(SubESPInfo, "Text & Data ESP")

CreateToggle(SecESPInfo, "Name ESP", "Zeigt den Namen des Spielers.", false, function(state) Config.ESPName = state end)
CreateToggle(SecESPInfo, "Character Moveset ESP", "Zeigt das Moveset des Spielers.", false, function(state) Config.ESPCharacter = state end)
CreateToggle(SecESPInfo, "Distance ESP", "Zeigt die Distanz in Studs.", false, function(state) Config.ESPDistance = state end)
CreateToggle(SecESPInfo, "Kill ESP", "Zeigt Kills des Spielers an.", false, function(state) Config.ESPKill = state end)
CreateToggle(SecESPInfo, "HP Text", "Zeigt exakte HP Zahlen.", false, function(state) Config.ESPHPText = state end)
CreateToggle(SecESPInfo, "HP Bar", "Zeigt einen grafischen Lebensbalken.", false, function(state) Config.ESPHPBar = state end)
CreateToggle(SecESPInfo, "Cooldown Revealer", "Zeigt gegnerische Cooldowns und Ultimates.", false, function(state) Config.CooldownRevealer = state end)
...
CreateKeybind(SecLockMain, "Lock Keybind", Enum.KeyCode.C, function(key) Config.LockKeybind = key end)
CreateToggle(SecLockMain, "Lock ESP", "Zeigt einen Ring um das gelockte Ziel.", false, function(state) Config.LockESP = state end)

local srvDropLock = CreateDropdown(SecLockMain, "Target Player", {"Auto"}, "LockSpecificPlayer", function(val) 
    Config.LockSpecificPlayer = val
    if val ~= "Auto" then Logic.LockState.CurrentLockTarget = nil; Logic.LockState.LastTargetSearch = 0 end
end)
Config._LockSpecificPlayerRefresh = srvDropLock.Refresh

CreateButton(SecLockMain, "Spieler Liste Aktualisieren", Theme.SectionBG, function() refreshPlayerDropdown(); RyuNotify:Send("Target Lock", "Spielerliste aktualisiert!", 2) end)
CreateToggle(SecLockMain, "Wand-Check (Wall Check)", "Lockt nicht durch Wände.", false, function(state) Config.LockWallCheck = state end)
CreateToggle(SecLockMain, "Auto Retarget", "Sucht automatisch ein neues Ziel, wenn das alte stirbt.", false, function(state) Config.LockAutoRetarget = state end)
CreateToggle(SecLockMain, "Lock Notifications", "Zeigt Popups beim Locken.", true, function(state) Config.LockNotification = state end)

local SubLockConfig = CreateSubTab(TabLock, "Konfiguration")
local SecLockConfig = CreateSection(SubLockConfig, "Erweiterte Settings")
CreateDropdown(SecLockConfig, "Method", {"Camera", "Body"}, "LockMethod", function(val) Config.LockMethod = val end)
CreateDropdown(SecLockConfig, "Target Mode", {"Closest", "Closest to Mouse"}, "LockTargetMode", function(val) Config.LockTargetMode = val end)
CreateDropdown(SecLockConfig, "Target Part", {"Head", "HumanoidRootPart", "UpperTorso", "LowerTorso"}, "LockTargetPart", function(val) Config.LockTargetPart = val end)
CreateSlider(SecLockConfig, "Glätte (Smoothness)", 0, 20, 0, function(val) Config.LockSmoothness = val end)
CreateSlider(SecLockConfig, "Kamera Seiten-Offset", -8, 8, 1.75, function(val) Config.LockSideOffset = val end)
CreateSlider(SecLockConfig, "Max Lock Distanz", 10, 2000, 500, function(val) Config.LockMaxDistance = val end)
CreateSlider(SecLockConfig, "Prediction", 0, 1, 0, function(val) Config.LockPrediction = val end)

-- TAB 4: TELEPORTS
local TabTeleports = CreateMainTab("Teleports")
local SubTeleports = CreateSubTab(TabTeleports, "Reisen (Travel)")
local SecTeleports = CreateSection(SubTeleports, "Auto Teleport Settings")

CreateDropdown(SecTeleports, "Zielort (Destination)", LocationNames, "TeleportDestination", function(val) Config.TeleportDestination = val end)
CreateDropdown(SecTeleports, "Teleport Methode", {"Tween", "Tween + Invis"}, "TeleportMode", function(val) Config.TeleportMode = val end)
CreateSlider(SecTeleports, "Teleport Speed (Studs/s)", 30, 1500, 350, function(val) Config.TeleportSpeed = val end)
CreateToggle(SecTeleports, "Zeige Pfad an", "Macht den berechneten Pfad sichtbar.", true, function(state) Logic.Pathfinding.VisualizeOn = state end)

CreateButton(SecTeleports, "Start Teleport", Theme.SectionBG, function()
    local destName = Config.TeleportDestination
    local destPos = TeleportLocations[destName]
    if destPos then
        if Config.TeleportMode == "Tween + Invis" then Invisibility.toggle(true, false) end
        RyuNotify:Send("Teleport", "Pathfinding zu " .. destName .. " läuft...", 2)
        startPathfinding(destPos)
    end
end)
CreateButton(SecTeleports, "Stopp Teleport", Theme.SectionBG, function()
    stopPathfinding()
    RyuNotify:Send("Teleport", "Teleport manuell gestoppt.", 2)
end)
...
CreateToggle(SecSideDash, "Aktiviere Side Dash Assist", "Macht einen Bogen-Dash zum Gegner.", false, function(state) Config.EnableDashAssist = state; Logic.DashAssistState.Enabled = state end)
CreateToggle(SecSideDash, "Lock Camera On Enemy", "Lockt die Kamera beim Dash.", false, function(state) Config.DashCameraLock = state end)
CreateToggle(SecSideDash, "Nur wenn man nach vorne schaut", "Dasht nur, wenn du den Gegner ansiehst.", false, function(state) Config.DashOnlyIfFacing = state end)
CreateKeybind(SecSideDash, "Dash Keybind", Enum.KeyCode.J, function(key) Config.DashAssistKeybind = key end)
CreateSlider(SecSideDash, "Erkennungs-Reichweite", 1, 50, 15, function(val) Config.DashDetectionRange = val end)
CreateSlider(SecSideDash, "Distanz hinter Gegner", 1, 15, 5, function(val) Config.DashBehindDistance = val end)
CreateSlider(SecSideDash, "Flugdauer (sek)", 0.1, 1.0, 0.42, function(val) Config.DashFlightDuration = val end)

local SubDashConfig = CreateSubTab(TabSideDash, "Arc Modifiers")
local SecDashConfig = CreateSection(SubDashConfig, "Kurven & Bogen")
CreateSlider(SecDashConfig, "Kurvenstärke", 0, 25, 10, function(val) Config.DashCurveStrength = val end)
CreateSlider(SecDashConfig, "Bogenhöhe (Arch Height)", 0, 10, 3, function(val) Config.DashArchHeight = val end)
CreateSlider(SecDashConfig, "Lock Dauer (sek)", 0.1, 1.5, 0.35, function(val) Config.DashLockDuration = val end)
...
local SrvInfoLabel = CreateLabel(SecServerInfo, "Kein Server ausgewählt.\n\nKlicke 'Server Liste aktualisieren' um Server zu laden,\ndann wähle einen aus dem Dropdown.")
SrvInfoLabel.Name = "ServerDetailsLabel"

CreateButton(SecServerInfo, "Server Beitreten", Theme.SectionBG, function()
    if not Logic.SelectedServerData then RyuNotify:Send("Servers", "Kein Server ausgewählt!", 3); return end
    if Logic.SelectedServerData.IsCurrent then RyuNotify:Send("Servers", "Du bist bereits in diesem Server!", 3); return end
    RyuNotify:Send("Servers", "Teleportiere...", 3)
    TeleportService:TeleportToPlaceInstance(game.PlaceId, Logic.SelectedServerData.JobId, Player)
end)
CreateButton(SecServerInfo, "Bestem Ping Server beitreten", Theme.SectionBG, function()
    if #Logic.ServerList == 0 then RyuNotify:Send("Servers", "Aktualisiere zuerst die Liste!", 3); return end
    ...
    if best then RyuNotify:Send("Servers", "Trete bestem Ping Server bei (" .. best.Ping .. "ms)...", 3); TeleportService:TeleportToPlaceInstance(game.PlaceId, best.JobId, Player)
    else RyuNotify:Send("Servers", "Kein Server gefunden!", 3) end
end)
CreateButton(SecServerInfo, "Kleinstem Server beitreten", Theme.SectionBG, function()
    if #Logic.ServerList == 0 then RyuNotify:Send("Servers", "Aktualisiere zuerst die Liste!", 3); return end
    ...
    if smallest then RyuNotify:Send("Servers", "Trete kleinstem Server bei (" .. smallest.Playing .. " Spieler)...", 3); TeleportService:TeleportToPlaceInstance(game.PlaceId, smallest.JobId, Player)
    else RyuNotify:Send("Servers", "Kein Server gefunden!", 3) end
end)
CreateButton(SecServerInfo, "Rejoin Current Server", Theme.SectionBG, function()
    RyuNotify:Send("Servers", "Server wird neu betreten...", 3)
    TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, Player)
end)

local SecServerStats = CreateSection(SubServerList, "Server Statistiken")
local SrvStatsLabel = CreateLabel(SecServerStats, "Keine Daten. Bitte aktualisieren.")
SrvStatsLabel.Name = "ServerStatsLabel"

local SecServerList = CreateSection(SubServerList, "Liste & Filter")
CreateButton(SecServerList, "Server Liste Aktualisieren", Theme.SectionBG, function()
    task.spawn(function()
        local sList = fetchServers()
        local vals = buildServerMap(sList)
        if #vals > 0 then Config.ServerDropdown = vals[1] else Config.ServerDropdown = "No servers found" end
        if Config._ServerDropRefresh then Config._ServerDropRefresh(#vals > 0 and vals or {"No servers found"}) end
        updateStatsLabel()
    end)
end)

local srvDropList = CreateDropdown(SecServerList, "Wähle Server", {"Klicke 'Aktualisieren'"}, "ServerDropdown", function(val)
    Config.ServerDropdown = val
    Logic.SelectedServerData = Logic.ServerMap[val]
    updateServerInfo(Logic.SelectedServerData)
end)
Config._ServerDropRefresh = srvDropList.Refresh

CreateDropdown(SecServerList, "Sortieren Nach", {"Default (Roblox)", "Most Players", "Least Players", "Lowest Ping", "Highest Ping", "Highest FPS"}, "ServerSortBy", function(Value)
    ...
    if Config._ServerDropRefresh then Config._ServerDropRefresh(vals) end
end)
CreateSlider(SecServerList, "Min. Spieler Filter", 0, 50, 0, function(val) Config.MinPlayersFilter = val end)
CreateSlider(SecServerList, "Max Ping Filter", 10, 999, 999, function(val) Config.MaxPingFilter = val end)
CreateButton(SecServerList, "Filter Anwenden", Theme.SectionBG, function()
    if #Logic.ServerList == 0 then RyuNotify:Send("Servers", "Aktualisiere zuerst die Liste!", 3); return end
    local minP = Config.MinPlayersFilter; local maxPing = Config.MaxPingFilter
    ...
    if Config._ServerDropRefresh then Config._ServerDropRefresh(#vals > 0 and vals or { "Keine Server gefunden" }) end
    RyuNotify:Send("Servers", "Zeige " .. #vals .. " Server nach Filterung.", 3)
end)
...
CreateButton(SecUI, "Unload UI (Löscht das GUI)", Theme.SectionBG, function()
...
pcall(function()
    if Tabs[1] and Tabs[1].Btn then
        Tabs[1].IsOpen = true; Tabs[1].SubContainer.Size = UDim2.new(1, 0, 0, Tabs[1].SubLayout.AbsoluteContentSize.Y)
        Tabs[1].Btn.TextColor3 = Theme.Text; Tabs[1].Btn.BackgroundColor3 = Theme.SectionBG
    end
